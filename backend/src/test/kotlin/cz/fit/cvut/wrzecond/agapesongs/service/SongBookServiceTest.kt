...
class SongBookServiceTest: StringSpec ({
    ...
    val song1 = Song("Kéž se všichni svatí", "Text 1", SongKey.E.name, 120, 0, timeStampNow, 1, sb1, 1)
    val song2 = Song("Hava nagila", "Text 2", SongKey.B_FLAT.name, 90, 0, timeStampNow, null, sb1, 2)
    val song3 = Song("Služme Pánu s bázní", "Text 3", SongKey.D.name, 130, 1, timeStampNow, 1, sb2, 3)
    ...
    val rawSbDto = SongBookReadDTO(sb1.id, sb1.name, BandReadDTO(band1.id, band1.name, emptyList()), emptyList())

    val songBookDto1 = SongBookReadDTO(sb1.id, sb1.name, rawSbDto.band, listOf(
        SongReadDTO(song2.id, rawSbDto, song2.name, listOf(), SongKey.valueOf(song2.key), song2.bpm, song2.capo, song2.lastEdit, song2.displayId, noteDto),
        SongReadDTO(song1.id, rawSbDto, song1.name, listOf(), SongKey.valueOf(song1.key), song1.bpm, song1.capo, song1.lastEdit, song1.displayId, null)
    ))
    val songBookDto2 = SongBookReadDTO(sb2.id, sb2.name, BandReadDTO(band2.id, band2.name, emptyList()), listOf(
        SongReadDTO(song3.id, SongBookReadDTO(sb2.id, sb2.name, BandReadDTO(band2.id, band2.name, emptyList()), emptyList()),
            song3.name, listOf(), SongKey.valueOf(song3.key), song3.bpm, song3.capo, song3.lastEdit, song3.displayId, null),
    ))
