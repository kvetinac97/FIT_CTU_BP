...
class SongServiceTest: StringSpec ({
    ...
    val createDto = SongCreateDTO("Kéž se všichni svatí", "Text 1", SongKey.E, 120, 2, songBook.id, 1)
    ...
    val song = Song("Kéž se všichni svatí", "[]", SongKey.E.name, 120, 1, timeStampNow, 1, songBook, 1)
    val updateDto = SongUpdateDTO("Hava nagila", null, SongKey.B_FLAT, 90,  null, null, null)
    ...
    val songDto = SongReadDTO(song.id, SongBookReadDTO(song.songBook.id, song.songBook.name,
        BandReadDTO(band1.id, band1.name, emptyList()), emptyList()), song.name,
        listOf(), SongKey.valueOf(song.key), song.bpm, song.capo, timeCreate, song.displayId, null)
