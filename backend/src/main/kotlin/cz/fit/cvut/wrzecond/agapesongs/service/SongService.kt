...
class SongService (override val repository: SongRepository, val songBookRepository: SongBookRepository, userRepository: UserRepository)
    ...
    override fun Song.toDTO () = SongReadDTO(id, songBook.toDTO(), name, convertSongText(text),
        SongKey.valueOf(key), bpm, capo, lastEdit, displayId, null)
    override fun SongCreateDTO.toEntity () = Song(name, convertTextToJson(text), key.name, bpm, capo,
        Timestamp.from(Instant.now()), displayId, songBookRepository.getById(songBookId))
