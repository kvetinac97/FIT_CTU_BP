...
import cz.fit.cvut.wrzecond.agapesongs.dto.SongKey
...
class AuthService(override val repository: UserRepository, private val bandRepository: BandRepository,
    ...
    private fun createUser (email: String, name: String) : User {
        val user = repository.saveAndFlush(User(generateLoginSecret(), email, name, emptyList(), emptyList()))
        val band = bandRepository.saveAndFlush(Band("$name's band", "[]", emptyList(), emptyList()))
        val role = roleRepository.getById(1)
        bandMemberRepository.saveAndFlush(BandMember(band, user, role))
        val songBook = songBookRepository.saveAndFlush(SongBook("Showcase", emptyList(), band))
        songRepository.saveAndFlush(Song("Showcase song", "[{\"id\":\"h3V350jQ\",\"chords\":\"C          G\",\"text\":\"This is an example of a song\"},{\"id\":\"oWSHKHzj\",\"text\":\"It is a showcase\"},{\"id\":\"bfO11zsP\",\"text\":\"\"},{\"id\":\"iwSCQbvk\",\"chords\":\"F       C            G\",\"text\":\"This is how a chorus can look like\"},{\"id\":\"0rK4t6ji\",\"text\":\"You can edit it as you like\"},{\"id\":\"s3NURBEU\",\"text\":\"\"},{\"id\":\"NIaLFtkB\",\"text\":\"\"}]",
            SongKey.C.name, 999, 0, Timestamp.from(Instant.now()), null, songBook))
        return user
    }
    // === PRIVATE METHODS ===

    private data class AppleToken (val id_token: String)
    private data class AppleUserInfo (val email: String, val email_verified: Boolean)
